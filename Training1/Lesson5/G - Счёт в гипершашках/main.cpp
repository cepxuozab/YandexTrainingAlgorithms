...
/*
    ...
    далее по отсортированным ключам словаря найти методом двух указа
    се такие отрезки отсортированной последовательности ключей, 
    правая граница которых не подходит под условие left * k <= right
    т.е. для каждой левой границы найти правую границу - первую карточку, которая не подходит под условия задачи 
    циентом k
    ...
    что делать, если некоторые карточки повт
    ?
    рассмотрим варианты:
    ...
    т.е. добавятся 3 * (количество value
...
int main() {
    ...
    for (size_t left = 0; left < uniqueNums.size(); ++left) {
        while (right < uniqueNums.size() && k * uniqueNums[left] >= uniqueNums
ght]) { //останавливаемся на первом неподходящем элементе
